...
export default {
  getProductById: async (req: Request, res: Response) =>
    ProductService.getPhoneBy<string>('id', String(req.params.productId || ''))
      .then((result) =>
        res
          .status(result.find ? 200 : 404)
          .json(
            result.find ? result.data : { error: 'Product has not found. ' }
          )
          .end()
      )
      .catch((error) => res.status(500).json({ error }).end()),

  getRecommendedById: async (req: Request, res: Response) =>
    ProductService.getPhoneBy<string>(
      'id',
  ...
  getDiscountedProducts: async (req: Request, res: Response) =>
    ProductService.getProductList({
      page: 1,
      limit: 8,
      sortBy: 'discountPercentage',
      sortType: 'desc',
      where: Sequelize.where(Sequelize.literal('"fullPrice" - "price"'), {
        [Op.gt]: 0,
      }),
    }).then((result) => res.status(200).json(result).end()),
